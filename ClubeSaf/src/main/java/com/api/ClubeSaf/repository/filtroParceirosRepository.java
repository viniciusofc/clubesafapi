...
public interface filtroParceirosRepository extends JpaRepository<filtroParceiros, Long>{
	...
	@Query(value = "select * from gr_parceiros par,gr_segmentos seg where  par.id_segmento=seg.id_segmento and par.id_empresa= ?1 and UPPER(par.cidade) = UPPER(?2) and par.id_segmento= ?3 order by par.prioridade desc, par.parceiro", nativeQuery = true)
